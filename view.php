...
if (!$course = $DB->get_record('course', array('id' => $courseid))) {
    print_error('invalidcourse', 'local_filescan', $courseid);
}
 
 
require_login($course);
$PAGE->set_url('/local/filescan/view.php', array('id' => $courseid));
$PAGE->set_pagelayout('standard');
$PAGE->set_heading(get_string('viewdetailspage', 'local_filescan'));
...
$canview = has_capability('local/filescan:viewpages', $context);
...
/**
 ...
 function create_file_item($DB, $cm, $cmtype, $mod_name, $sectionName, $section_no, $file) {
 	...
 	// See if there is an file scan entry for this file
		...
		$results = $DB->get_record("local_filescan_files", array('contenthash'=>$file->get_contenthash()), $fields='*');
